...
def pair_sampling(blocking_function,
    ...
    :param blocking_threshold: int or None
        ...
        -   int; if the blocking function is block_phonetic
            ...
            author_disambiguation.clustering.blocking_funcs.py
    ...
    clusters_reversed = {v: k for k, va in true_clusters.items()
                         for v in va}
    ...
    for _, sig_s in blocking_dict.items():
    ...
    if balanced:
        ...
        all_pairs = map(lambda x: int(math.ceil(
                        category_size / float(len(x)))) * x,
                        [dasn, sasn])
        pairs = reduce(lambda x, y: x + random.sample(y, int(category_size)),
                       all_pairs, [])
    else:
        # positive = sasn + sadn
        # negative = dasn + dadn
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_pairs", default="my_pairs.json", type=str)
