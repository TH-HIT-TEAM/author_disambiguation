...
global mesh2cui, cui2vec


def mesh_to_cui():
    """get mesh terms map to cui``.
    ...
    global mesh2cui
    mesh2cui = {}
    with codecs.open("../../data/mesh_to_cui.txt", "r", "utf-8") as f:
        for line in f:
            line = line.strip()
            if line:
                mesh = line.split("\t")[0]
                cui = line.split("\t")[1]
                if mesh not in mesh2cui:
                    mesh2cui[mesh] = cui
    global cui2vec
    cui2vec = {}
    with codecs.open("../../data/re_rel_par_chi_sb_cui2vec.txt", "r", "utf-8") as f:
        for line in f.readlines():
            line = line.strip()
            cui = line.strip().split()[0]
            cui2vec[cui] = []
            for vect in line.split()[1:]:
                cui2vec[cui].append(float(vect))
...
def get_mesh_word2vec(s):
    """Get year from the word2vec file.
    ...
    -------
    :returns: np.array
        all mesh terms vector sum
    """
    global mesh2cui, cui2vec
    v = s["mesh_terms"]
    mesh_list =[m.split(":")[1].strip() for m in v.strip().split(";") if m]
    sum = np.zeros((200,), dtype=object)
    for mesh in mesh_list:
        if mesh in mesh2cui and mesh2cui[mesh] in cui2vec:
            vec = np.asarray(cui2vec[mesh2cui[mesh]])
            sum = sum + vec
    # print(sum)
    return str(sum.tolist())
